...
# If the first file is not present in the working directory the script prints a message the program will stop
...
blue='\e[34m' # Blue colour for text
clear='\e[0m' # clear colour
if [[ $# -ne 2 || ! $1 =~ .csv$ || ! $2 =~ .csv$ ]]; then # Checks if the input is valid is not then TRUE
...
if [[ ! -f $1 ]]; then # Checks if the file to be parsed is in the working directory
    echo "Cannot find any file named $1"
    exit 0
fi

if [[ -f $2 ]]; then # Checks if the output file is in the working directory or not
    rm "$2" # Removes the file if exists
fi

original_file=$1 # Assigns first argument to original_file 
processed_file=$2 # Assigns second argument to processed_file
